...
async def agendamentos(request: Request, page: int = 1, order_by: str = 'id', order: str = 'asc', per_page: int = 10):
...
def get_items(order_by: str = 'id', order: str = 'asc', page: int = 1, per_page: int = 10) -> tuple | None:
    ...
    db = SessionLocal()
    try:
        query = db.query(Agendamento).order_by(getattr(Agendamento, order_by).desc() if order == 'desc' else getattr(Agendamento, order_by))
        total = query.count()
